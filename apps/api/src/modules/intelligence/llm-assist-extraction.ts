...
export async function assistExtractionGaps(options: {
  ...
  if (!isLlmModelLive(model) || model.provider === 'caprov' || model.alwaysAvailable) {
    return { facts, filled: [] };
  }

  const missing = GAP_KEYS.filter((key) => !facts.some((fact) => fact.key === key && fact.value));
  if (!missing.length) {
    return { facts, filled: [], note: 'Deterministic extractor already complete; LLM assist skipped.' };
  }
  ...
  try {
    ...
    for (const key of missing) {
      ...
      const haystack = documents.map((document) => document.extractedText ?? '').join('\n').toLowerCase();
      if (!haystack.includes(proposed.fragment.toLowerCase().slice(0, 40))) {
        continue;
      }
      if (!haystack.includes(String(proposed.value).toLowerCase().slice(0, 24))) {
        continue;
      }
      const source = documents.find((document) =>
        (document.extractedText ?? '').toLowerCase().includes(proposed.fragment.toLowerCase().slice(0, 40)),
      );
      if (!source) continue;
      ...
      if (
        ...
        (source.type === 'INSURANCE' || isNonMarketValueFragment(fragment) || isNonMarketValueFragment(proposed.value))
      ) {
        continue;
      }
      if (['market_value', 'nav'].includes(key) && !['VALUATION_MEMO', 'FINANCIAL_STATEMENT'].includes(source.type)) {
      ...
      let confidence = Math.min(0.86, Number((proposed.confidence ?? 0.8).toFixed(2)));
      ...
      next.push({
        ...
        numericValue: parsed?.amount ?? parsePercent(proposed.value) ?? parseYears(proposed.value),
        currency: parsed?.currency,
        unit: parsePercent(proposed.value) != null && !parsed ? '%' : parseYears(proposed.value) != null ? 'years' : undefined,
        confidence,
        provenance: [
          {
...
async function callJsonAssist(
  ...
  corpus: string,
): Promise<Record<string, { value: string; fragment: string; confidence?: number }> | null> {
...
Prefer valuation memos and financial statements for marks. Documents:\n${corpus}`;
  ...
  if (model.provider === 'anthropic') {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        model: model.id.includes('haiku') ? 'claude-3-5-haiku-latest' : 'claude-sonnet-4-20250514',
        max_tokens: 800,
    ...
    return parseJsonObject(payload.content?.map((part) => part.text ?? '').join('\n') ?? '');
  }
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'Return only valid JSON. Never invent numbers not present in the documents.',
        },
...
function parseJsonObject(
  text: string,
): Record<string, { value: string; fragment: string; confidence?: number }> | null {
